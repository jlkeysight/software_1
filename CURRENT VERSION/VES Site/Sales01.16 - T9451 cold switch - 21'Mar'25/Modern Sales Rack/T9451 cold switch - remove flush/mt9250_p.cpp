...
void t9255(int mod_id, int /*slot*/)
{
  ...
  OpenResourceManager();//chun 11012022
  open_dmm(mod_id);
  open_34980A(mod_id);//chun 11012022
...
//mjs -- changed bit number from 14 to 1 to reflect changes from pascal
 ...
 /* disconnect_34980A_relay(1026);
  disconnect_34980A_relay(1027);
  disconnect_34980A_relay(2912);
  disconnect_34980A_relay(2001);
  disconnect_34980A_relay(2036);
  disconnect_34980A_relay(2009);
  disconnect_34980A_relay(2044);
  disconnect_34980A_relay(3007);
  disconnect_34980A_relay(3008);
  disconnect_34980A_relay(3042);
  disconnect_34980A_relay(3043);
  disconnect_34980A_relay(1001);
  disconnect_34980A_relay(1002);
  disconnect_34980A_relay(1004);
  disconnect_34980A_relay(1005);*/

  close_dmm(mod_id);

  close_34980A(mod_id);//chun11012022
  CloseResourceManager();//chun11012022
...
void t9256(int mod_id, int /*slot*/)
{
  ...
  OpenResourceManager();//chun11012022
  open_dmm(mod_id);
  open_34980A(mod_id);//chun11012022
  //if (get_34980A_IDN() == true)//chun24112021
  //{
	 // open_34980A(mod_id);
  //}
...
void t9260(int mod_id, int /*slot*/)
{
  ...
  open_dmm(mod_id);
  open_34980A(mod_id);
  /*if (get_34980A_IDN() == true)//chun07122021 temp disable to run test with 34980A
  {
	  open_34980A(mod_id);
  }
  else
  ...
  close_34980A(mod_id);//chun enable back on 03032022
  CloseResourceManager();
  //if (get_34980A_IDN() == true)//chun 24112021  //temp disable due to use 34980A
  //{
	 // close_34980A(mod_id);
  //}
  //else
...
#define OLYM_WAIT_TIME			150e-03//1.0e-00//150e-03 //added by chuanho, 14jun2012
...
void t9265(int mod_id_, int /*slot*/)
{
  ...
  //connect_dmm(V.mod_id);//chun26012022 added this line to allow run 9265 independently
...
#define FINEST_INCREMENT    8e-03
#define SYN_WAIT_TIME       10e-03
//#define OLYM_WAIT_TIME      2.0e-00 //added by chuanho, 14jun2012
...
#define OLYM_WAIT_TIME	      1.5e-00 //150e-03 //added by chuanho, 14jun2012
#define OLYM_CH1_CURR		  500e-03 //added by chuanho, 14jun2012
#define OLYM_CH1_OCP		  1e+00 //added by chuanho, 14jun2012
...
void t9267(int mod_id, int /*slot*/)
{
  ...
  vth_mtmr_wait(mod_id, 1.5); /* add for stabilize new Fun Gen 3360A Fikri 19-July-2022*/
  ...
  vth_mtmr_wait(mod_id, 10.0); /* add for stabilize new Fun Gen 3360A Fikri 19-July-2022*/
  flush_commands(mod_id);
...
void t9268(int mod_id, int /*slot*/)
{
 ...
 /* if (get_34980A_IDN() == true)
  ...
  vth_mtmr_wait(mod_id, 5.0); /* add for stabilize new Fun Gen 3360A Fikri 19-July-2022*/
...
void t9270(int mod_id, int /*slot*/)
{
  ...
  test_result_type result;
  OpenResourceManager(); //chun added on 04032022
  open_cntr(mod_id);
  open_34980A(mod_id);
  //open_mux(mod_id);
...
*******************************************************************/
  ...
  //display_prompt("   Before enter aconnect_osc_ch1_af2", true); // quah 4/3/25 check 34980A GPIB error
  aconnect_osc_ch1_af2(mod_id);
//display_prompt("   After enter aconnect_osc_ch1_af2", true); // quah 4/3/25 check 34980A GPIB error
...
*******************************************************************/
  ...
  disconnect_34980A_relay(2009);
  disconnect_34980A_relay(2044);
  disconnect_34980A_relay(5101);
  disconnect_34980A_relay(5401);
  disconnect_34980A_relay(5402);
  disconnect_34980A_relay(5403);
  disconnect_34980A_relay(5404);*/

  close_cntr(mod_id);
  close_34980A(mod_id);//close_mux(mod_id);
  CloseResourceManager();//chun added on 04032022
}  /* t9270 */
...
#define VINIT           1.01e+00
#define EXPECTED_DET_VOLTS  2.0e+00
#define TIMEOUT         1.0e+00
#define PS_CURRENT      800e-03
#define DIFF            5e-03
#define RAMP_TIME       10e+00
#define LOW_CURRENT_VALUE  27.5e-03  //28.0e-03  //fikri simulation
#define HIGH_CURRENT_VALUE  165e-03
...
void t9274(int mod_id, int /*slot*/)
{
  ...
  OpenResourceManager();//chun added on 040302022
  vth_apowered_init(mod_id);
  vth_mtmr_wait(mod_id, SALES_RACK_WAIT);
  ...
  vth_aopen_rly(mod_id, K820);   /* DETLO to L bus relay */
  vth_aopen_rly(mod_id, K812);   /* DETHI to I bus relay */
  vth_mtmr_wait(mod_id, 1); //fikri 15052023
  flush_commands(mod_id); //fikri 15052023
  vth_mtmr_wait(mod_id, 1); //fikri 15052023
  ...
  vth_afc_en_src_out(mod_id);
  //setup_ps(mod_id, ps_voltage, PS_CURRENT);
  setup_PS_N6746B(mod_id, ps_voltage, PS_CURRENT);
  vth_mtmr_wait(mod_id, 1.01);
  //verify_ps_output(mod_id, DIFF, RAMP_TIME, fabs(ps_voltage));
  verify_PS_N6746B_output(mod_id, DIFF, RAMP_TIME, fabs(ps_voltage));
  vth_mtmr_wait(mod_id, 1.01);
  vth_astat_clr(mod_id);
  ...
  if (get_result_nolog(mod_id, &result))
   detector_voltage = result.UU.Real_result;
//	double x_1= detector_voltage; //11Aug2022 print result
//	printf ("%f \n",x_1);}
  else
    display_prompt("No result in result log (Expected DET volts)", true);
  if (!get_result_nolog(mod_id, &result))
  ...
  if (get_result_nolog(mod_id, &result))
  vload = fabs(result.UU.Real_result);
//	double x_1= vload; //11Aug2022 print result
//	printf ("%f \n",x_1);}
  else
    display_prompt("No result in log(Expected DET volts)", true);
  if (!get_result_nolog(mod_id, &result))
  ...
  if (get_result_nolog(mod_id, &result))
  vnoload = fabs(result.UU.Real_result);
// 	double x_1= vnoload; //11Aug2022 print result
//	printf ("%f \n",x_1); }          //11Aug2022 print result
 else
    display_prompt("No result in log(Expected DET volts)", true);
  if (!get_result_nolog(mod_id, &result))
    display_prompt("No result in log (Expected DET stat)", true);
...
void t9277(int mod_id, int /*slot*/)
{
  ...
  OpenResourceManager();//chun added
  vth_apowered_init(mod_id);
  vth_mtmr_wait(mod_id, SALES_RACK_WAIT);
  //open_ps(mod_id);
  open_PS_N6746B(mod_id);
  ...
  {  /* SETUP FOR POSITIVE AND NEGATIVE SOURCE VOLTAGES */
    ...
    {  /* SETUP FOR SOURCING AND SINKING CONDITIONS */
	  ...
	  setup_PS_N6746B(mod_id, PS_VOLTAGE_[src_polarity]
		  [direction], POWER_SUPPLY_CURRENT);
      vth_mtmr_wait(mod_id, POWER_SUPPLY_RAMP_TIME);
  ...
  close_PS_N6746B(mod_id);
  close_34980A(mod_id);//close_mux(mod_id);
  CloseResourceManager();//chun added
}  /* t9277 */
#undef SOURCING
#undef SINKING
#undef POSITIVE
...
#define OLYM_FINEST_INCREMENT  10e-03//added by chuanho, 14jun2012
#define SYN_WAIT_TIME   10e-03
#define OLYM_WAIT_TIME 150e-03//1.5e-00//150e-03 //added by chuanho, 14jun2012
#define OLYM_CH1_CURR  500e-03 //added by chuanho, 14jun2012
#define OLYM_CH1_OCP  1e+00 //added by chuanho, 14jun2012
#define RESOLUTION      1e-03
...
void t9317(int mod_id, int /*slot*/)
{
  ...
  flush_commands(mod_id);
   vth_mtmr_wait(mod_id, 1.0); /* add for stabilize new Fun Gen 3360A Fikri 19-July-2022*/
  setup_olym_ch1_volt(mod_id, INTERNAL_SRC_VOLTAGE);
   vth_mtmr_wait(mod_id, 1.0); /* add for stabilize new Fun Gen 3360A Fikri 19-July-2022*/
  setup_olym_ch1_curr(mod_id, OLYM_CH1_CURR);
   vth_mtmr_wait(mod_id, 1.0); /* add for stabilize new Fun Gen 3360A Fikri 19-July-2022*/
...
#define DIFF            5e-03
#define RAMP_TIME       10e+00
#define LOW_CURRENT_VALUE  27.5e-03 //sri
#define HIGH_CURRENT_VALUE  165e-03
...
//mjs -- another messed up bit problem
  ...
  /*disconnect_34980A_relay(1021);
  disconnect_34980A_relay(2911);
  disconnect_34980A_relay(2001);
  disconnect_34980A_relay(2036);
  disconnect_34980A_relay(2009);
  disconnect_34980A_relay(2044);*/
...
void t9327(int mod_id, int /*slot*/)
{
  ...
  {  /* SETUP FOR POSITIVE AND NEGATIVE SOURCE VOLTAGES */
    ...
    {  /* SETUP FOR SOURCING AND SINKING CONDITIONS */
      ...
      //         [direction], POWER_SUPPLY_CURRENT);
	  setup_PS_N6746B(mod_id, PS_VOLTAGE___[aux_polarity]
		  [direction], POWER_SUPPLY_CURRENT);
      vth_mtmr_wait(mod_id, POWER_SUPPLY_RAMP_TIME);
...
void t9355(int mod_id, int /*slot*/)
{
  ...
  OpenResourceManager();//chun added on 01032022
  open_dmm(mod_id);
  //open_mux(mod_id);
  open_34980A(mod_id);
...
//mjs -- another messed up bit problem
  ...
  close_dmm(mod_id);
  close_34980A(mod_id);//close_mux(mod_id);
  CloseResourceManager();//chun added on 01032022
}  /* t9355 */
...
int frequency;
  ...
  OpenResourceManager();//chun added 01032022
  open_dmm(mod_id);
  //open_mux(mod_id);
  open_34980A(mod_id);
  ...
  close_dmm(mod_id);
  close_34980A(mod_id);//close_mux(mod_id);
  CloseResourceManager();//chun addded 01032022
}  /* t9360 */
#undef NUM_FREQUENCIES_TO_TEST
#undef F128
#undef F1024
...
void t9365(int mod_id, int /*slot*/)
{
  ...
  OpenResourceManager();//chun 07032022
  open_cntr(mod_id);
  //open_mux(mod_id);
  open_34980A(mod_id);
  ...
  close_cntr(mod_id);
  close_34980A(mod_id);//close_mux(mod_id);
  CloseResourceManager();//chun 07022022
}  /* t9365 */
#undef NUM_FREQUENCIES_TO_TEST
#undef SAMPLE_SIZE
#undef F128
...
/*
  ...
  close_dmm(mod_id);
  close_34980A(mod_id);//close_mux(mod_id); //9371
  CloseResourceManager();//chun addded 01032022
//  if (ofile != NULL)
//    fclose(ofile);
}
...
//  ofile = NULL;
  /* connect DMM to aim source */
  OpenResourceManager();//chun_added
  open_dmm(mod_id);
  open_34980A(mod_id);
  //open_mux(mod_id);
...
/*
  ...
  close_34980A(mod_id);//close_mux(mod_id); //9372
  CloseResourceManager();
//  if (ofile != NULL)
//    fclose(ofile);
}
...
/*        t1signal = sum_power(lo1_index, hi1_index - 1, y);
  ...
  close_dmm(mod_id);
  close_34980A(mod_id);//close_mux(mod_id); //9380
...
//    fclose(ofile);
  ...
  close_dmm(mod_id);
  close_34980A(mod_id);//close_mux(mod_id); //9383
  CloseResourceManager();//chun addded 01032022
}
#undef NUM_FREQS_TO_TEST
#undef DCSETOFF
...
void t9386(int mod_id, int /*slot*/)
{
  ...
  OpenResourceManager();//chun added on 07032022
  open_dmm50(mod_id);
  //open_mux(mod_id);
  open_34980A(mod_id);
...
********************************************************************/
  ...
  close_dmm50(mod_id);
  close_34980A(mod_id);//close_mux(mod_id);
  CloseResourceManager();//chun added on 07032022
}  /*t9386*/
#undef ARB_FREQ
#undef SRC_I_COMPLIANCE
#undef SRC_R_OUT
...
#define RMS_VOLTS       7.071e+00 //fikri
#define DC_OFFSET       0.0e+00
/*seconds*/
#define SOURCE_WAIT_TIME  1.0e-03
...
/*	  {
		  ...
		  {
			 result.Measurement_class = CLASS_REAL;
			 result.UU.Real_result = (largest - smallest)-0.0002;
			 log_result(mod_id, result);
...
int this_frequency;
  ...
  OpenResourceManager();//chun 07022022
...
//    vth_mtmr_wait(mod_id, 1.0); /* add for stabilize new Fun Gen 3360A Fikri 19-July-2022*/
    ...
    {  /* FOR CURRENT - SETUP AND MEASURE EACH CASE */
      ...
      {  /* FOR SOURCE POLARITY */
        ...
        measure_dmm(mod_id); //subtest 0,5,10,15

		if (get_result_nolog(mod_id, &result))
		{

/////////////////////////////new code 20/09/2022-FIkri-////////////////////////////////////////////////////
		 if (source_polarity == POSITIVE)
		 {
//	        dmm_no_load_voltage = result.UU.Real_result+0.04;  //added offset -+0.04V Fikri 13-Sep-2022
	        dmm_no_load_voltage = result.UU.Real_result;  //added offset -+0.04V Fikri 13-Sep-2022
             result.UU.Real_result = dmm_no_load_voltage;
			log_result(mod_id, result);
		 }
...
//original			dmm_no_load_voltage = result.UU.Real_result;  //original code 20/09/2022 fikri
		}
        else
          display_prompt("No result in result log (Expected DMM v)", true);
       ...
       vth_mtmr_wait(mod_id, WAIT_TIME_FOR_RELAYS);
        ...
        {  /* FOR DIRECTION := SOURCING TO SINKING */
		  ...
		  setup_PS_N6746B(mod_id, low_v, IPS);
		  ...
		  setup_PS_N6746B(mod_id, high_v, IPS);
          vth_mtmr_wait(mod_id, PS_RAMP_TIME);
          flush_commands(mod_id); //added fikri 15/5/2024
          vth_mtmr_wait(mod_id, 3.0); /* add for stabilize new Fun Gen 33250 Fikri 26/06/2024*/
		  ...
		  setup_PS_N6746B(mod_id, vps, IPS);
          vth_mtmr_wait(mod_id, PS_RAMP_TIME);

          /* MEASURE THE OUTPUT CURRENT AND LOG THE RESULT */
          flush_commands(mod_id); //added fikri 15/5/2024
          vth_mtmr_wait(mod_id, 3.0); /* add for stabilize new Fun Gen 33250 Fikri 26/06/2024*/
           measure_dmm(mod_id); //subtest 1,3,6,8,11,13,16,18
          if (get_result_nolog(mod_id, &result))
          {  /* get result */
...
/////////////////////////////new code 20/09/2022-FIkri-////////////////////////////////////////////////////
		...
		measure_dmm(mod_id); //subtest 2,4,7,9,12,14,17,19

		if (get_result_nolog(mod_id, &result))
		{

		 if (source_polarity == POSITIVE){
//	        double dmm_read= result.UU.Real_result+0.04;  //added offset -+0.04V Fikri 13-Sep-2022
	        double dmm_read= result.UU.Real_result;  //added offset -+0.04V Fikri 13-Sep-2022
            result.UU.Real_result = dmm_read;
			log_result(mod_id, result);
		}
...
//			dmm_no_load_voltage = result.UU.Real_result;  //original code 20/09/2022 fikri
		}
        else
          display_prompt("No result in result log (Expected DMM v)", true);
/////////////////////////////new code 20/09/2022-Fikri-////////////////////////////////////////////////////
...
//fikri 14092022
/*		if (result.UU.Real_result>=0){
	    	double val1 = result.UU.Real_result+0.05;  //added offset -0.04V Fikri 13-Sep-2022
            result.UU.Real_result = val1;
		}
...
*/ //fikri 14092022
    ...
    //close_ps(mod_id);
	close_PS_N6746B(mod_id);
    vth_mtmr_wait(mod_id, 0.5);   /* Required for Coto relays so that   */
	close_dmm(mod_id);
    vth_mtmr_wait(mod_id, 0.5);   /* Required for Coto relays so that   */
    close_34980A(mod_id);//close_mux(mod_id);
	vth_mtmr_wait(mod_id, 0.5);   /* Required for Coto relays so that   */
...
//    if ( !number_equal(ever_had_a_vlimit && voltage_increment, FINEST_INCREMENT) )
  ...
  vth_mtmr_wait(V.mod_id, 10.0); /* add for stabilize new Fun Gen 3360A Fikri 19-July-2022*/
...
void t9427(int mod_id, int /*slot*/)
{
  ...
  {  /* SETUP FOR POSITIVE AND NEGATIVE SOURCE VOLTAGES */
    ...
    {  /* SETUP FOR SOURCING AND SINKING CONDITIONS */
      ...
      //         [direction], POWER_SUPPLY_CURRENT);
	  setup_PS_N6746B(mod_id, PS_VOLTAGE_____[src_polarity]
		  [direction], POWER_SUPPLY_CURRENT);
      vth_aopen_rly(mod_id, K2605);   /* MOA happy relay */
      vth_mtmr_wait(mod_id, POWER_SUPPLY_RAMP_TIME);
      vth_afunc_meas(mod_id);
      ...
      if (get_result_nolog(mod_id, &result))
	  {
		  ...
		  {
			  max_current = (result.UU.Real_result / r10);
			  result.UU.Real_result = fabs(max_current)+0.03;
			  log_result(mod_id, result);

		  }

		  else if(subtest_no==2)
		  {
			  max_current = (result.UU.Real_result / r10);
			  result.UU.Real_result = fabs(max_current)+0.03;
			  log_result(mod_id, result);
	...
	//	  max_current = (result.UU.Real_result / r10)+0.04;
	//	result.UU.Real_result = fabs(max_current);
      //  log_result(mod_id, result);
// 	    double x_1= result.UU.Real_result; //11Aug2022 print result
//        printf (" current= %f\t r10_cal= %f \n",x_1, r10);           //11Aug2022 print result
//        printf ("%f\n",x_1);           //11Aug2022 print result
	  subtest_no++;
	  }
      else
...
#define F1024           6
#define F8192           7
#define R_RATIO         226 //227 simulation
...
void t9440(int mod_id, int /*slot*/)
{
  ...
  vth_mtmr_wait(mod_id, 1); //fikri 15052023
      ...
      for (int voltage = 0; voltage <= 1; voltage++)
      {  /* for dc voltage */
    ...
    close_34980A(mod_id);//close_mux(mod_id);
        ...
        flush_commands(mod_id); //fikri 15052023
        vth_mtmr_wait(mod_id, 1); //fikri 15052023
    CloseResourceManager();//chun 07032022
  }  /* After check for slot 2 pin card */
...
//  char nasru_p10_part_no[10] = { 'N', '1', '8', '0', '7', '6', '6', '8', '2', '0'};   //bllack relay
 ...
 //   sprintf(buffer, "  Unable to read revision code.");
  ...
  //for (int counter = 0; counter < 10 && nasru_p10_id==1; counter++) //chnaged from 10->9 characters to check
    for (int counter = 0; counter < 9 && nasru_p10_id==1; counter++)
...
#define RESOLUTION      1e-03
#define WAIT_TIME       10.0e00 //60e-3
...
void t9465(int mod_id, int /*slot*/)
{
  ...
  /*Connect the synthesizer output to the X1 and X2 and the CALIB lines to dmm*/
   vth_mtmr_wait(mod_id, 2.0); /* add for stabilize new Fun Gen 3360A Fikri 19-July-2022*/
  connect_dmm(mod_id);
   vth_mtmr_wait(mod_id, 2.0); /* add for stabilize new Fun Gen 3360A Fikri 19-July-2022*/
  connect_plus_calib1(mod_id);
  vth_mtmr_wait(mod_id, 2.0); /* add for stabilize new Fun Gen 3360A Fikri 19-July-2022*/
