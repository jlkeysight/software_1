...
void t16000(void)
{
	...
	//char banner1[81] = "----------------------------------------------------";
	//char banner2[81] = "													"; 
	char banner3[81] = "	Keyight Technologies";
	//char banner4[81] = "	MSD In-Circuit Test system";
	//char banner5[81] = "	i5000 Sales Tests 01.09";
	//char banner6[81] = "	For Windows PC";				                            
	//char banner[81] = "--------------------------------------------------";
	...
	//printf("DGN version is: %s \n", buffer);

	display_prompt(banner3, true);
	/*display_prompt(banner1, true);
...
//	display_prompt("---------------------------------------------------	", true);
    ...
    if(check_syn_model())
	{
	  close_syn(mod_id);
      /*System PS N6700B*/
	  display_prompt("Verifying N6700B PS ...", true);
	  open_n6700b(mod_id);
	  verify_n6700b(mod_id);
	  close_n6700b(mod_id);
	...
	open_dmm50(mod_id);
	verify_dmm50(mod_id);
	if(is_dmm50_rear_input(mod_id))
	{
		close_dmm50(mod_id);

		/* GENERATOR */
		display_prompt("Verifying PULSE GENERATOR ...", true);
		open_gen(mod_id);
		verify_gen(mod_id);
		close_gen(mod_id);
	}
	...
	/* Load PS */
	//Remarked by Chuan Ho (09-Oct-2015) due to not all Sales rack standardized with 6038A PS installation, alternate way is to check utility card presence if to run this
    /*(display_prompt("Verifying LOAD PS ...", true);
	open_ps_load();
	display_prompt("Load Power supply okay", true);
    close_ps_load();
	*/
...
void t16010(int mod_id, int /* slot */)
{
 	int bus;
	double farads;
 	int fixture_id; // added sri on the 13th April 2007, 
	test_result_type result;
	...
	fixture_id = get_fixture_id(mod_id); // added SRI on the 13th April 2007
	if (fixture_id == ANA_ID_MAX)		// added SRI on the 13th April 2007
			{
				printf("SALES ANALOG FIXTURE #3 FOUND, CORRECTION REQUIRED/n");
			}
	...
	{  /*for*/
		...
		farads = farads - (0.45e-10);//rack stability 13th March 2007

		
		// SRI ADDED FOR SALES ANALOG FIXTURE #3 ON THE 13TH April 2007
        ///////////////////////////////////////////////////
		if (fixture_id == ANA_ID_MAX)
			{
				farads = farads + (1.00e-10);
			}
	...
	vth_mtmr_wait(mod_id, WAIT_TIME);
	farads = measure_lcr(mod_id);
	farads = farads - (0.1e-10);//rack stability 13th March 2007
...
#undef LCR_CKT_MODE
  ...
  //########### Sri added the new test 16011 and 16012 = Utility Load Test ########################################
...
#define DIFF1           15e-03 //need to undefine
#define RAMP_TIME       100e+00//need to undefine
...
#define PS_CURRENT_LOAD  3.0    
  ...
  //------------------------------------------------------------------------------
void t16011(int mod_id, int slot)//Channel 7 DUT PS Load Test/
{
  ...
  OpenResourceManager(); //added OCH, 24May2011
  open_mux(mod_id);
  open_ps_load();
  open_ps(mod_id); 
...
*/
	...
	result.UU.Real_result = setup_ps_load(mod_id,30,3);
    log_result(mod_id, result);
...
// ############################## end of new test 16011 added ##########################################



void t16012(int mod_id, int slot)//Channel 8 DUT PS Load Test/
{
...
#undef R
	...
	{/*for: side*/
		...
		//////////////////////////////////////////////////////////////////////////
		if ((fixture_id == ANA_ID_MAX) && (p==0||p==1||p==2||p==3))		// added SRI on the 13th April 2007
			{
				farads = farads + (2.8e-10);
			}
		//printf("SriTharan/n");
		//////////////////////////////////////////////////////////////////////////
